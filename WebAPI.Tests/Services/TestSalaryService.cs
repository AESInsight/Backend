...
namespace WebAPI.Tests.Services
{
    ...
    public class TestSalaryService
    {
        ...
        public async Task GetLatestSalaryForEmployeeAsync_ReturnsLatestSalary()
        {
            // Arrange
            var employeeId = 1;
            var salary1 = new SalaryModel { EmployeeID = employeeId, Salary = 50000, Timestamp = DateTime.UtcNow.AddDays(-2) };
            var salary2 = new SalaryModel { EmployeeID = employeeId, Salary = 60000, Timestamp = DateTime.UtcNow.AddDays(-1) };
        ...
        [Test]
        public async Task GetLatestSalaryForEmployeeAsync_ReturnsLatestSalary_WhenMultipleSalariesExist()
        {
            // Arrange
            var employeeId = 1;
            var salary1 = new SalaryModel { EmployeeID = employeeId, Salary = 50000, Timestamp = DateTime.UtcNow.AddDays(-2) };
            var salary2 = new SalaryModel { EmployeeID = employeeId, Salary = 60000, Timestamp = DateTime.UtcNow.AddDays(-1) };
            var salary3 = new SalaryModel { EmployeeID = employeeId, Salary = 65000, Timestamp = DateTime.UtcNow };
            ...
            // Assert
            Assert.That(result, Is.Not.Null);
            Assert.That(result.Salary, Is.EqualTo(65000));
            Assert.That(result.Timestamp, Is.EqualTo(salary3.Timestamp).Within(TimeSpan.FromSeconds(1)));
        }

        [Test]
        public async Task GetLatestSalaryForEmployeeAsync_ReturnsNull_WhenNoSalariesExist()
        {
            // Act
